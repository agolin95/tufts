...
if @conversion.save
      ...
      if params[:conversion][:author] == ''
        params[:conversion][:author] = ' '
      end
      ...
      #insert pid
      pid = @doc.at_css "idno"
      if params[:conversion][:pid] == ''
        params[:conversion][:pid] = ' '
      end
      pid.content = params[:conversion][:pid]
      puts pid.content
      File.write("defaultxml.xml", @doc.to_xml)

      #TODO: THIS NEEDS TO BE FIXED, IT OVERWRITES <p> ELEMENT, not sure how to
      #select a specific <p> element nested in other element.
      #insert recording details
      recording = @doc.at_css "equipment"
      if params[:conversion][:recording] == ''
        params[:conversion][:recording] = ' '
      end
      ...
      if params[:conversion][:description] == ''
        params[:conversion][:description] = ' '
      end
